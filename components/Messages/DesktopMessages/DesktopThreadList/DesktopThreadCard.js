...
const DesktopThreadCard = ({ title, thread }) => {
  ...
  return (
    <li
      className={
        "mb-2 flex w-full flex-row rounded-lg bg-slate-200 p-3 hover:cursor-pointer hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700" +
        (isActive ? "  text-[#FC8D4D]" : "")
      }
      onClick={handleClickCard}
    >
      <div className="ml-3 flex w-11/12 flex-col pl-3">
        <span className="text-sm font-bold"> {parseTitle(title)[0]}</span>
      </div>
