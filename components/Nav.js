...
const Nav = ({ children }) => {
    ...
    return (
        ...
        <nav className="flex w-screen py-6 px-8">
            ...
            {/* desktop & large view */}
            <div className="hidden md:relative md:block w-1/3 text-gray-600 focus-within:text-gray-400">
                <span className="absolute left-0 top-1.5 flex items-center pl-2">
                    <GoSearch size={24} />
                </span>
                <input type="search" className="w-full py-2 text-sm text-white bg-gray-200 rounded-full pl-10 focus:outline-none focus:bg-white focus:text-gray-900" placeholder="Search..." autoComplete="off">
                    {children}
                </input>
            </div>
            ...
            <button className="flex items-center justify-center h-10 w-10 ml-2 p-2 rounded-full bg-white dark:bg-gray-900">
                {theme === 'light' ?
                <BsFillMoonStarsFill className="text-indigo-800 hover:text-indigo-500" size={24} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}/>
                :
                <BsFillSunFill className="text-yellow-200 hover:text-yellow-100" size={24} onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')}/>
                }
            </button>
