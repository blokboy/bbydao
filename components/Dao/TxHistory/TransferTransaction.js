...
const TransferTransaction = ({ tx, owners, threshold }) => {
        ...
        <div className="flex flex-row items-center justify-between rounded-lg bg-slate-300 p-2 shadow dark:bg-slate-900">
          <div className="flex flex-row">
            <span className="mr-1 flex w-20 flex-row justify-center rounded border border-white bg-slate-100 p-1 text-[12px] dark:bg-slate-800">
              <span>
                {ethers.utils.formatEther(tx.value ? tx.value : 0).slice(0, 8)}
              </span>{" "}
            ...
            <span className="flex w-20 items-center justify-center rounded border border-white bg-slate-100 p-1 text-[12px] text-yellow-500 dark:bg-slate-800">
              { tx?.receiver ? tx.receiver.slice(0, 4).concat("...").concat(tx.receiver.slice(tx.receiver.length - 4, tx.receiver.length)) : "TRANSFER" }
            </span>
          </div>
