    ...
    <>
      ...
      <div className="flex w-full flex-col overflow-auto pt-4 md:flex-row">
        <div className="flex-start flex flex-col px-4 md:w-3/12">
          <SidePanel nftImage={data?.collectibles[0]?.imageUri} safeInfo={data.safeInfo} />
        </div>
        <div className="flex-start item m-3 flex flex-col md:m-0 md:mr-1 md:w-full md:flex-row">
          ...
          <div className="flex w-full flex-col md:w-1/2">
            ...
            <TxHistory allTxs={data.allTxs} owners={data.safeInfo.owners} threshold={data.safeInfo.threshold} />
            <ProposalHistory />
          </div>

          {/* dao page modals  */}
          {osSellModalOpen && <SellModal safeAddress={data?.safeInfo.address} />}
          {followDaoModalOpen && <FollowModal safeAddress={data?.safeInfo.address} />}
          {txModalOpen && <TransactionModal safeAddress={data?.safeInfo.address} />}
          {editDaoMemberModalOpen && <EditDaoMemberModal safeAddress={data?.safeInfo.address} />}
          {uniswapLpModalOpen && <UniswapLpModal safeAddress={data?.safeInfo.address} tokenLogos={tokenLogos} />}
          {uniswapSwapModalOpen && <UniswapSwapModal safeAddress={data?.safeInfo.address} />}
        </div>
      </div>
    </>
  )
