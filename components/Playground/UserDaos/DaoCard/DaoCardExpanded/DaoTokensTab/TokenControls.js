...
const TokenControls = ({liquidityPair, token, isUniV2}) => {
    ...
    return (
        <>
            {!toggleControls && (
                <button
                    type="button"
                    className="flex justify-center shadow-xl rounded bg-slate-200 dark:bg-slate-800"
                    onClick={() => setToggleControls(state => !state)}>
                    <HiChevronDown size={24} />
                </button>
            )}
            <div className={`${!toggleControls ? 'h-0' : 'h-auto'} transition-all overflow-hidden`}>
                {(!isUniV2 && (
